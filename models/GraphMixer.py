...
from models.modules import TimeEncoder, MLPClassifier
from utils.utils import NeighborSampler

from models.gdn import graph_deviation_network
from models.supconloss import SupConLoss

class GraphMixer(nn.Module):

    def __init__(self, config,node_raw_features: np.ndarray, edge_raw_features: np.ndarray, neighbor_sampler: NeighborSampler,
        ...
        self.affinity_score = MLPClassifier(self.node_feat_dim, dropout=0.2)
        self.gdn = graph_deviation_network(self.cfg, device, self.node_feat_dim)
        self.suploss = SupConLoss()

    def forward(self,  src_node_ids: np.ndarray, node_interact_times: np.ndarray, labels: np.ndarray, num_neighbors: int = 20, time_gap: int = 2000):
        # apply tgat
        source_node_embedding = self.compute_src_dst_node_temporal_embeddings(src_node_ids, node_interact_times, num_neighbors, time_gap)

        anom_score = self.gdn(source_node_embedding, node_interact_times, labels)  # 异常检测
        dev, group = self.gdn.dev_diff(torch.squeeze(anom_score), node_interact_times, labels)

        # 节点分类
        logits = self.affinity_score(source_node_embedding)

        prediction_dict = {}
        prediction_dict['logits'] = torch.reshape(logits, [-1])
        prediction_dict['anom_score'] = anom_score
        prediction_dict['time'] = torch.from_numpy(node_interact_times)
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray,
                                                 node_interact_times: np.ndarray,num_neighbors: int = 20, time_gap: int = 2000, mode= None):
        """
    ...
    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
        ...
        # if mode == 'aug':
        #     neighbor_node_ids, neighbor_edge_ids, neighbor_times = drop_edge(neighbor_node_ids,neighbor_edge_ids,neighbor_times, 0.2)
            
        # Tensor, shape (batch_size, num_neighbors, edge_feat_dim)
        # edge_raw_features (num_edges + 1, edge_feat_dim)
        # neighbor_edge_ids (batch_size, num_neighbors)
        # =>Tensor, shape (batch_size, num_neighbors, edge_feat_dim)
    ...
    def set_neighbor_sampler(self, neighbor_sampler: NeighborSampler):
        ...
        """
        self.neighbor_sampler = neighbor_sampler
    def set_node_edge_feature(self):
        return 1
