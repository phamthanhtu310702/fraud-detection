...
#             w = torch.nn.Parameter(torch.ones(*param))
#             torch.nn.init.kaiming_normal_(w)
#             vars.append(w)
#             vars.append(torch.nn.Parameter(torch.zeros(param[0])))
...
#     def __init__(self, args, hidden_dim: int, device):
...
#         self.device = device
#         self.memory = torch.rand_like(torch.zeros(self.memory_size, dtype= torch.float32, requires_grad=False)).to(device)
...
class graph_deviation_network(torch.nn.Module):
    def __init__(self, args, device, hidden_dim):
        super(graph_deviation_network, self).__init__()
        self.net = AnomalyLayer(hidden_dim)
        self.memory_size = args.memory_size # 5000 # 4000
        self.sample_size = args.sample_size # 2000 # 1000
